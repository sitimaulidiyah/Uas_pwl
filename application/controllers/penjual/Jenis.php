...
class Jenis extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->template->load('penjual/dashboard', 'penjual/jenis/data', $data);
    }
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            $data['title'] = "Jenis";
            $this->template->load('penjual/dashboard', 'penjual/jenis/add', $data);
        } else {
            $input = $this->input->post(null, true);
            $insert = $this->admin->insert('jenis', $input);
            if ($insert) {
                set_pesan('data berhasil disimpan');
                redirect('penjual/jenis');
            } else {
                set_pesan('data gagal disimpan', false);
                redirect('penjual/jenis/add');
            }
        }
    }
    ...
    public function edit($getId)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->template->load('penjual/dashboard', 'penjual/jenis/edit', $data);
        } else {
            $input = $this->input->post(null, true);
